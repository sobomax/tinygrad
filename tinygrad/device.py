...
class _Device:
  ...
  def DEFAULT(self) -> str:
    device_from_env: Optional[str] = functools.reduce(lambda val, ele: ele if getenv(ele) == 1 else (f"{ele}:{getenv(ele)}"
                                                                           if getenv(ele) > 1 else val), self._buffers, None)   # type: ignore
